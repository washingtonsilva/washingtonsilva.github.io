...
A @fig-fdp ilustra a fdp de $X$. Analisando a @fig-fdp, podemos deduzir 
que a densidade de $X$ depende também do suporte definido por $\theta$, 
ou seja, se alguma realização $x_i$ estiver fora do intervalo permitido, 
a densidade de $X$ e, por consequência, a função de verossimilhança podem 
ser zero.

```{r}
#| label: fig-fdp
#| fig-align: center
#| fig-cap: "Função densidade de probabilidade de X."
#| echo: false
...
de máxima verossimilhança de $\theta$ é **máximo** (a $n$-ésima estatística de ordem) d
a amostra.
...
Segundo @hansen2022ps, talvez isso não seja surpreendente. Ao definir 
...
A @fig-cons ilustra a consistência da densidade $U[0,\hat \theta]$ com os dados citada 
por @hansen2022ps. A densidade uniforme de $X$ só é válida se cobrir todos os dados. Entre as opções consistentes 
($\theta \geq M_n$), a escolha $\theta = M_n$ tem a maior altura e, portanto, maximiza a verossimilhança.
